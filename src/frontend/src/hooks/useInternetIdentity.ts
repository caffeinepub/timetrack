...
export function InternetIdentityProvider({
  ...
  // Use useRef to avoid re-triggering the init effect when authClient changes
  const authClientRef = useRef<AuthClient | undefined>(undefined);
  // Capture createOptions in a ref so the init effect can read the latest value without re-running
  ...
  const clear = useCallback(() => {
    ...
    void authClient
      ...
      .then(() => {
        ...
        setStatus("idle");
        setError(undefined);
        // Re-initialize the auth client after logout so the app is ready for a new login
        void (async () => {
          try {
  ...
  // This effect runs ONCE on mount. authClientRef is not a dependency so it never re-runs.
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
  ...
  }, []); // intentionally empty — runs only once on mount (createOptions captured via ref)

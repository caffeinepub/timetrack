...
export function InternetIdentityProvider({
  ...
  // Store createOptions in a ref so the init effect can access it without re-running
  ...
  // Run once on mount — authClientRef and createOptionsRef are stable refs, not state
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
        ...
        const existingClient = authClientRef.current;

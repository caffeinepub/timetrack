 ...
 */
export function isDateInWeek(timestamp: bigint, weekStart: Date, weekEnd: Date): boolean {
  const date = new Date(Number(timestamp) / 1000000);
  const dateOnly = new Date(date.getFullYear(), date.getMonth(), date.getDate());
  const startOnly = new Date(weekStart.getFullYear(), weekStart.getMonth(), weekStart.getDate());
